...
export function Home() {
  ...
  return (
    <div>
      {user ? <WorkerHero name={profile?.username ?? user.email?.split('@')[0] ?? 'there'} /> : <LandingHero />}
      <FeaturesStrip />
      <section className="max-w-6xl mx-auto px-4 sm:px-6 py-16">
        <SectionHeader title="Work spots in Baguio" sub={`${spots.length} listed`} href="/spots" />
        {topSpots.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {topSpots.map((spot, i) => (
              <div key={spot.id} className={`animate-fade-up s${Math.min(i + 1, 6)}`}>
                <SpotCardHydrated spot={spot} />
              </div>
            ))}
          </div>
        ) : (
          <p className="font-mono text-sm text-dim py-12 text-center">No spots listed yet.</p>
        )}
        {spots.length > 8 && (
          <div className="mt-8 text-center">
            <Link to="/spots" className="btn-secondary">View all {spots.length} spots <ArrowRight size={14} /></Link>
          </div>
        )}
      </section>
      {!user && <JoinBanner />}
  ...
  return (
    <section className="relative overflow-hidden border-b border-border">
      ...
      <div className="absolute inset-0 opacity-[0.04]" style={{
        backgroundImage: 'linear-gradient(#c8c3b8 1px, transparent 1px), linear-gradient(90deg, #c8c3b8 1px, transparent 1px)',
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 py-28 md:py-36">
        <div className="max-w-3xl">
          ...
          <p className="text-lg text-dim leading-relaxed mb-10 max-w-xl">
            Cafes rated by remote workers on WiFi speed, power outlets, noise level, and more. Not vibes — data.
          </p>
        ...
        <div className="mt-20 pt-8 border-t border-border grid grid-cols-3 gap-8 max-w-sm">
          {[['6', 'Work metrics'], ['0', 'Paid listings'], ['100%', 'Community rated']].map(([v, l]) => (
            <div key={l}>
  ...
  return (
    <section className="border-b border-border bg-raised">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12 flex items-center justify-between gap-6 flex-wrap">
        <div>
          <p className="font-mono text-2xs text-amber uppercase tracking-widest mb-2">Welcome back</p>
          <h1 className="font-display text-3xl text-head">Good to see you, {name}.</h1>
          <p className="text-sm text-dim mt-1">Where are you working from today?</p>
        </div>
        <div className="flex gap-3">
          <Link to="/review/new" className="btn-primary">+ Write a review</Link>
          <Link to="/spots" className="btn-secondary">Browse spots</Link>
        </div>
      </div>
    </section>
  )
...
function FeaturesStrip() {
  const items = [
    { icon: Wifi,    label: 'WiFi Speed',     desc: 'Rated for video calls and uploads' },
    { icon: Zap,     label: 'Power Outlets',  desc: 'Per seat availability' },
    { icon: Volume2, label: 'Noise Level',    desc: 'Focus-friendliness score' },
    { icon: Monitor, label: 'Laptop Setup',   desc: 'Glare, desk depth, ergonomics' },
  ]
  return (
  ...
  return (
    <section className="border-t border-border bg-raised">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-20 text-center">
        <p className="font-mono text-2xs text-amber uppercase tracking-widest mb-4">Community-powered</p>
        <h2 className="font-display text-4xl text-head mb-4">You know a good spot.</h2>
        <p className="text-dim text-lg mb-8">Rate a cafe. Help the next nomad find their focus zone.</p>
