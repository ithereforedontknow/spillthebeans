...
import { fetchUserPassport, fetchVisitedSpots, fetchPassportLeaderboard } from '@/lib/supabase/queries'
import { PageSpinner } from '@/components/ui/Spinner'
import { Avatar } from '@/components/ui/Avatar'
import { cn } from '@/lib/utils'
import { BADGE_TIERS, BADGE_REQUIREMENTS, BADGE_DESCRIPTIONS, type BadgeTier } from '@/types'
import { MapPin, Lock, Trophy } from 'lucide-react'

// Badge visual config
const BADGE_CONFIG: Record<BadgeTier, { color: string; bg: string; border: string; glyph: string }> = {
  Newcomer: { color: 'text-dim',     bg: 'bg-raised',       border: 'border-border',    glyph: '○' },
  Explorer: { color: 'text-body',    bg: 'bg-card',         border: 'border-muted',     glyph: '◎' },
  Regular:  { color: 'text-amber',   bg: 'bg-amber/5',      border: 'border-amber/30',  glyph: '◉' },
  Grinder:  { color: 'text-amber',   bg: 'bg-amber/10',     border: 'border-amber/50',  glyph: '★' },
  Veteran:  { color: 'text-amber',   bg: 'bg-amber/15',     border: 'border-amber',     glyph: '✦' },
}
...
export function Passport() {
  ...
  if (!user) return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="text-center">
        <h2 className="font-display text-3xl text-head mb-3">Your Coffee Passport</h2>
        <p className="text-dim mb-6">Sign in to track the spots you've worked from.</p>
  ...
  const nextTier     = BADGE_TIERS[BADGE_TIERS.indexOf(currentTier) + 1] as BadgeTier | undefined
    ...
    <div className="min-h-screen">
      ...
      <div className="border-b border-border bg-raised relative overflow-hidden">
        <div className="absolute inset-0 opacity-[0.03]" style={{
          backgroundImage: 'radial-gradient(#f0a500 1px, transparent 1px)',
          backgroundSize: '24px 24px',
        }} />
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 py-12">
          <div className="flex items-start gap-5 flex-wrap">
            <Avatar name={profile?.username ?? user.email} imageUrl={profile?.avatar_url} size="xl" />
            <div className="flex-1 min-w-0">
              <p className="font-mono text-2xs text-amber uppercase tracking-widest mb-1">Coffee Passport</p>
              ...
              <div className="flex items-center gap-2 mt-3">
                ...
                {passport?.avg_score_given && (
                  <span className="font-mono text-xs text-dim">
                    Avg score given: <span className="text-amber">{Number(passport.avg_score_given).toFixed(1)}</span>
                  </span>
                )}
              </div>
              ...
              {nextTier && (
                <div className="mt-4 max-w-xs">
                  ...
                  <div className="score-bar-track h-1.5">
                    <div className="score-bar-fill" style={{ width: `${Math.min(progress, 100)}%` }} />
                  </div>
                  <p className="font-mono text-2xs text-dim mt-1">
                    {nextTarget! - spotsVisited} more {nextTarget! - spotsVisited === 1 ? 'spot' : 'spots'} to reach {nextTier}
                  </p>
                </div>
              )}
              {!nextTier && (
                <p className="font-mono text-xs text-amber mt-2">Maximum tier reached.</p>
              )}
            </div>
            ...
            <div className="grid grid-cols-3 gap-6 text-center">
              {[
                ...
                [passport?.avg_score_given ? Number(passport.avg_score_given).toFixed(1) : '—', 'Avg score'],
              ].map(([v, l]) => (
                <div key={l}>
              ...
              <div key={tier} className={cn(
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className={cn('font-medium', earned ? 'text-head' : 'text-dim')}>{tier}</p>
                    {current && (
                  ...
                  <p className="font-mono text-2xs text-dim">{BADGE_DESCRIPTIONS[tier]}</p>
                </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div>
            <h2 className="font-display text-xl text-head mb-4">
              Spots Visited
              <span className="font-mono text-sm text-dim font-normal ml-2">({spotsVisited})</span>
            </h2>
            {visited.length > 0 ? (
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {visited.map((v: any) => {
                  const spot = v.spots
                  if (!spot) return null
                  const img = spot.image_url ?? `https://picsum.photos/seed/${spot.id}/240/160`
                  return (
                    <Link key={v.spot_id} to={`/spot/${spot.id}`}
                      ...
                      <div className="p-2.5">
                        ...
                        <div className="flex items-center justify-between mt-0.5">
                          <div className="flex items-center gap-1 text-dim">
                            <MapPin size={10} />
                            <span className="font-mono text-2xs truncate">{spot.city}</span>
                          </div>
              ...
              <div className="card p-10 text-center">
                <MapPin size={24} className="text-muted mx-auto mb-3" strokeWidth={1.5} />
                <p className="font-display text-lg text-head mb-1">No spots yet</p>
                <p className="text-sm text-dim mb-4">Review a spot to start your passport.</p>
                <Link to="/spots" className="btn-primary btn-sm">Browse spots</Link>
              </div>
            )}
          </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border">
                      <th className="text-left py-2.5 px-4 font-mono text-2xs text-dim uppercase tracking-widest">#</th>
                      <th className="text-left py-2.5 px-4 font-mono text-2xs text-dim uppercase tracking-widest">User</th>
                      <th className="text-left py-2.5 px-4 font-mono text-2xs text-dim uppercase tracking-widest">Badge</th>
                      <th className="text-right py-2.5 px-4 font-mono text-2xs text-dim uppercase tracking-widest">Spots</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border">
                    {leaderboard.slice(0, 10).map((entry: any, i: number) => (
                      <tr key={entry.id} className={cn(
                        ...
                        <td className="py-3 px-4 font-mono text-xs text-dim w-8">{i + 1}</td>
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-2">
                            <Avatar name={entry.username} imageUrl={entry.avatar_url} size="xs" />
                            <span className={cn('text-sm', entry.id === user.id ? 'text-amber font-medium' : 'text-body')}>
                        ...
                        <td className="py-3 px-4">
                          <BadgePill tier={entry.badge_tier as BadgeTier} compact />
                        </td>
