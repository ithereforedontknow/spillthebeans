...
interface LocationState { spotId?: string; edit?: DbReview }
...
export function WriteReview() {
  ...
  const filteredSpots = spots.filter(s => s.name.toLowerCase().includes(spotQ.toLowerCase()))
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      navigate(spotId ? `/spot/${spotId}` : '/spots')
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-raised">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-12">
        ...
        {/* Header */}
        <div className="mb-8">
          <Link to={selectedSpot ? `/spot/${selectedSpot.id}` : '/spots'} className="btn-ghost btn-sm mb-4 -ml-2 font-mono">
            <ArrowLeft size={13} />Back
          </Link>
          <h1 className="font-display text-3xl text-head">
            {editing ? 'Edit review' : 'Rate a work spot'}
          </h1>
          <p className="text-sm text-dim mt-1">Rate each dimension 1–5. Your ratings help other nomads.</p>
        </div>
        ...
        <div className="space-y-5">
          ...
          <section className="card p-5">
            ...
            {selectedSpot ? (
              <div className="flex items-center justify-between p-3 bg-raised border border-amber/20 rounded">
                <div>
                  <p className="text-sm font-medium text-head">{selectedSpot.name}</p>
                  <p className="font-mono text-2xs text-dim">{selectedSpot.city}</p>
                </div>
                {!editing && (
                  <button onClick={() => { setSpotId(''); setSpotQ('') }} className="text-dim hover:text-body">
                    <X size={14} />
                  </button>
                )}
              </div>
              ...
              <div className="relative">
                <input value={spotQ} onChange={e => setSpotQ(e.target.value)} placeholder="Search for a spot..." className="input" />
                {spotQ && (
                  <div className="absolute top-full left-0 right-0 mt-1 bg-card border border-border rounded-lg shadow-2xl z-20 max-h-48 overflow-y-auto">
                    {filteredSpots.length > 0 ? filteredSpots.map(s => (
                      <button key={s.id} onClick={() => { setSpotId(s.id); setSpotQ('') }}
                        className="w-full text-left px-4 py-2.5 hover:bg-raised transition-colors">
                        <p className="text-sm text-head">{s.name}</p>
                        <p className="font-mono text-2xs text-dim">{s.city}</p>
                      </button>
                    )) : <p className="px-4 py-3 text-sm text-dim">No spots found.</p>}
                  </div>
                )}
              </div>
            )}
            {errors.spot_id && <p className="font-mono text-2xs text-red-400 mt-1">{errors.spot_id}</p>}
          </section>
          ...
          <section className="card p-5">
            ...
            {!allRated && (
              <p className="font-mono text-2xs text-dim mt-4">Rate all 6 dimensions to continue.</p>
            )}
          </section>
          ...
          <section className="card p-5">
            ...
            <div className="relative">
              ...
              />
              <span className={cn('absolute right-3 bottom-3 font-mono text-2xs', body.length > 900 ? 'text-amber' : 'text-dim')}>
                {body.length}/1000
              </span>
            </div>
            {errors.body && <p className="font-mono text-2xs text-red-400 mt-1">{errors.body}</p>}
          </section>

          {/* Tags */}
          <section className="card p-5">
            <p className="input-label">Tags <span className="normal-case font-sans font-normal text-dim text-xs">(optional, max 8)</span></p>
            <div className="flex gap-2 mb-3">
              <input value={tagInput} onChange={e => setTagInput(e.target.value)}
                onKeyDown={e => { if (e.key === 'Enter' || e.key === ',') { e.preventDefault(); addTag(tagInput) } }}
                placeholder="Type and press Enter..." maxLength={32} className="input flex-1 text-sm" />
              <button type="button" onClick={() => addTag(tagInput)} className="btn-secondary btn-sm font-mono">Add</button>
            </div>
            {/* Suggestions */}
            <div className="flex flex-wrap gap-1.5 mb-3">
              {SUGGESTED_TAGS.filter(t => !tags.includes(t)).slice(0, 8).map(t => (
                <button key={t} onClick={() => addTag(t)} className="tag cursor-pointer hover:border-dim hover:text-body transition-colors">
                  + {t}
                </button>
              ))}
            </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {tags.map(t => (
                  <span key={t} className="flex items-center gap-1 tag">
                    {t}
                    <button onClick={() => setTags(p => p.filter(x => x !== t))} className="hover:text-body"><X size={10} /></button>
                  </span>
                ))}
              </div>
            )}
          ...
          >
            ...
            {isPending ? (editing ? 'Updating...' : 'Publishing...') : (editing ? 'Update Review' : 'Publish Review')}
          </button>

          {!canSubmit && !isPending && (
            <p className="font-mono text-2xs text-dim text-center">
              {!spotId ? 'Select a spot.' : !allRated ? 'Rate all 6 dimensions.' : 'Write at least 30 characters.'}
            </p>
          )}
        </div>
      </div>
