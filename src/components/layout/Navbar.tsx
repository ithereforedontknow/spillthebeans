    ...
    <header className="sticky top-0 z-40 border-b border-border bg-base/90 backdrop-blur-md">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between gap-6">
        ...
        <Link to="/" className="flex items-center gap-2.5 group shrink-0">
          <div className="w-6 h-6 bg-amber rounded-sm flex items-center justify-center">
            <svg viewBox="0 0 16 16" width="14" height="14" fill="none">
              <rect x="3" y="2" width="6" height="12" rx="1" fill="#1a2314" />
              <rect
                x="10"
                y="5"
                width="3"
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                cn(
                  ...
                  isActive
                    ? "text-head bg-raised"
                    : "text-dim hover:text-body hover:bg-raised",
                )
              }
            >
              {Icon && <Icon size={13} />}
        ...
        <div className="hidden md:flex items-center gap-2 shrink-0">
          {user ? (
            <>
              <Link to="/review/new" className="btn-primary btn-sm">
                + Review
              </Link>
              <div className="relative">
                <button
                  onClick={() => setDrop((p) => !p)}
                  className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-card transition-colors"
                >
                  <Avatar
                  ...
                  />
                  <span className="text-sm text-body max-w-[100px] truncate">
        ...
        <button
          onClick={() => setOpen((p) => !p)}
          className="md:hidden p-2 text-dim hover:text-body"
        >
          ...
          <div className="px-4 py-3 space-y-0.5">
            {nav.map(({ to, label }) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    "flex items-center px-3 py-2.5 rounded text-sm",
                    isActive ? "text-head bg-card" : "text-dim",
                  )
                }
              >
                {label}
            ...
            {user ? (
              <>
                <NavLink
                  to="/review/new"
                  onClick={() => setOpen(false)}
                  className="flex items-center px-3 py-2.5 text-sm text-amber"
                >
                  + Write Review
                </NavLink>
                <NavLink
                  to="/profile"
                  onClick={() => setOpen(false)}
                  className="flex items-center px-3 py-2.5 text-sm text-dim"
                >
                  Profile
                </NavLink>
                <NavLink
              ...
              <Link
                to="/login"
                onClick={() => setOpen(false)}
                className="block px-3 py-2.5 text-sm text-amber"
              >
                Sign in
              </Link>
            )}
...
function DropItem({
  ...
  return (
    <Link
      to={to}
      onClick={onClick}
      className="flex items-center gap-2.5 px-4 py-2 text-sm text-body hover:bg-raised hover:text-head transition-colors"
    >
