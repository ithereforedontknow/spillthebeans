...
export function RatingInput({ ratingKey, value, onChange, error }: RatingInputProps) {
  return (
    <div>
      <div className="flex items-start justify-between mb-2">
        <div>
          ...
          <p className="text-xs text-dim mt-0.5">{RATING_DESCRIPTIONS[ratingKey]}</p>
        </div>
        <span className={cn('font-mono text-lg font-semibold leading-none', value > 0 ? 'text-amber' : 'text-muted')}>
          {value > 0 ? value : '—'}
        </span>
      </div>
