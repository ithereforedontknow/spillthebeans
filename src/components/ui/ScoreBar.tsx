...
export function ScoreBar({ label, ratingKey, value, showValue = true, size = 'md', className }: ScoreBarProps) {
  ...
  return (
    <div className={cn('flex items-center gap-3', className)}>
      {displayLabel && (
        <span className={cn('font-mono text-dim shrink-0', size === 'sm' ? 'text-2xs w-16' : 'text-xs w-24')}>
          {displayLabel}
        </span>
      )}
      ...
      {showValue && (
        <span className={cn('font-mono font-semibold text-amber shrink-0 tabular-nums', size === 'sm' ? 'text-2xs w-6 text-right' : 'text-xs w-7 text-right')}>
...
export function ScoreGrid({ size = 'md', className, ...scores }: ScoreGridProps) {
