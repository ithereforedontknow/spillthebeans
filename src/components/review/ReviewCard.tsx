...
export function ReviewCard({
  review, spotName, likeCount, isLiked, isOwner, isLoggedIn,
  onLike, showSpot, className,
}: ReviewCardProps) {
  ...
  const handleFlag = async () => {
    ...
    try {
      ...
      setFlagOpen(false)
      setReason('')
      toast('Review reported. Thanks for helping keep NomadCafe accurate.')
    } catch (err: any) {
    ...
    <article className={cn('card p-5 animate-fade-up', className)}>
      ...
      <div className="flex items-center justify-between mt-4 pt-3 border-t border-border">
        <div className="flex items-center gap-2">
          ...
          {isLoggedIn && !isOwner && (
            flagged ? (
              <span className="flex items-center gap-1 px-3 py-1.5 font-mono text-xs text-amber/60">
